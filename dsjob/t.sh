...
Day1)
    ...
    dsjob -run -warn 0 -wait -param prmFilTarget=$fn.$datets.TXT -param prmDirTarget=$dir \
      ...
      -param prmDteTimestamp="$ts" -param prmDteStartDT=$3 -param prmDteEndDT=$4 \
	$proj $job 2>/dev/null
;;
Date1)
...
1)  # RUN OK (1)
    ...
    scp $log $logsumm $sftpuser@$monhost:/cygdrive/d/Hsbc_Project_Oracle/dsjob/$datedt/ >/dev/null 2>&1
